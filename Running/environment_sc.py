...
dtype = np.float32
class Env_cellular():
    def __init__(self, MAX_EP_STEPS, s_dim, location_vector, location_GF, Emax, K, B, T, eta, Pn, Pmax, w_d, w_csk, fading_n, fading_0):
    ...
    def step(self, action, state, j):
        ...
        En = state[0, -1]
        # En_bar = action * min(self.emax - En, (self.T * self.eta * self.Pn * hn0) - self.w_d) - (1 - action) * min(En, self.T * self.Pmax + self.w_csk)  # EQ 28
        En_bar = action * min(self.emax - En, (self.T * self.eta * self.Pn * hn0) - self.w_d) - (1 - action) * min(En, self.T * self.Pmax + self.T * self.w_csk)
        mu1 = self.eta * self.Pn * hn0 * h0 / (1 + self.Pn * hn)
        mu2 = ((En_bar + self.w_d) * h0) / (self.T * (1 + self.Pn * hn))
        ...
        # alpha01 = ((self.T * self.eta * self.Pn * hn0) - self.w_d - En_bar) / (self.T * self.eta * self.Pn * hn0 + self.T * self.w_csk + self.Pmax)  # P8(e)
        alpha01 = ((self.T * self.eta * self.Pn * hn0) - self.w_d - En_bar) / (self.T * self.eta * self.Pn * hn0 + self.T * self.w_csk + self.T * self.Pmax)
        # alpha02 = ((self.T * self.eta * self.Pn * hn0) - self.w_d - En_bar - En) / ((self.T * self.eta * self.Pn * hn0) + self.T * self.w_csk)  # P8(f)
        alpha02 = ((self.T * self.eta * self.Pn * hn0) - self.w_d - En_bar - En) / ((self.T * self.eta * self.Pn * hn0) - self.T * self.w_csk)
        ...
        if En_bar == self.T * self.eta * self.Pn * hn0:
            ...
            P0n = (1 - alphan) * self.eta * self.Pn * hn0 - self.w_d / alphan - En_bar / alphan / self.T - self.w_csk / self.T
    ...
    def TD3(self, action, state, j):
        ...
        return reward, state_next, EHTD3, done
    ...
    def PPO(self, action, state, j):
        ...
        return reward, state_next, EHPPO, done
