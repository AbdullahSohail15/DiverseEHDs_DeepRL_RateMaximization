...
dtype = np.float32
class Env_cellular():
    def __init__(self, MAX_EP_STEPS, s_dim, location_vector, location_GF, Emax, K, B, T, eta, Pn, Pmax, w_d, w_egc, w_csk, fading_n, fading_0):
    ...
    def step(self, action, state, j):
        ...
        En = state[0,-1]
        En_bar = action * min(self.emax - En, (self.T * self.eta * self.Pn * hn0) / self.B - self.w_egc - self.w_d) - (1 - action) * min(En, self.T * self.Pmax + self.w_csk)
        mu1 = self.T * self.eta * self.Pn * hn0 * h0 / (self.B * self.T * (1 + self.Pn * hn))
        mu2 = ((En_bar + self.w_egc + self.w_d) * h0) / (self.B * self.T * (1 + self.Pn * hn))
        mu3 = self.w_csk * h0 / (self.B*(1 + self.Pn * hn))
        wx0 = np.real(lambertw(math.exp(-1) * (1 - mu1 - mu3), k=0))
        alphaxx = (mu1 - mu2) / (math.exp(wx0 + 1) - 1 + mu1 + mu3)
        alpha01 = ((self.T * self.eta * self.Pn * hn0) - self.w_egc - self.w_d - En_bar) / (self.T * self.eta * self.Pn * hn0 + self.T * self.w_csk + self.B * self.Pmax)
        alpha02 = ((self.T * self.eta * self.Pn * hn0) - self.w_egc - self.w_d - En_bar - En) / ((self.T * self.eta * self.Pn * hn0) + self.T * self.w_csk - self.T * self.B * self.w_csk)
        ...
        if En_bar == self.T * self.eta * self.Pn * hn0:
            ...
            P0n = (1 - alphan) * self.eta * self.Pn * hn0 - self.w_egc - self.w_d / self.B * alphan - En_bar /self.B * alphan/self.T - self.w_csk
        ...
        EHD = (1 - alphan) * self.eta * self.T * self.Pn * hn0 / self.B
        done=False
